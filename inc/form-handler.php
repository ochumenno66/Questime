...
function questime_process_form(): void
{
  ...
  $now = time();
  $rate_data = array_filter($rate_data, fn($timestamp) => ($now - $timestamp) < 60);
  ...
  // Данные формы
  $form_type = sanitize_text_field(wp_unslash($_POST['form_type'] ?? 'default'));
  ...
  if ($form_type !== 'gamified') {
    ...
    if (!is_email($email)) {$errors['email'] = 'Enter correct email';}
    if (!$agree) {$errors['agree'] = 'You must agree to Privacy Policy';}
    if (!empty($errors)) {
      wp_send_json_error([
        'errors' => $errors
        ], 422);
  ...
  // Получатель из кастомайзера
  $to = get_theme_mod('questime_leads_email', get_option('admin_email'));
...
$headers = [
  ...
  'From: Questime <info@questime.com>',
  'Reply-To: ' . $email,
];
